...
public class T7BlockStateProvider extends BlockStateProvider {
  ...
  protected void registerStatesAndModels() {
    this.simpleBlockWithItem(T7Blocks.INSTANCE.getCRUCIBLE().get(), this.models().getBuilder(T7Blocks.INSTANCE.getCRUCIBLE().getId().getPath())
        ...
        .texture("particle", Thavma.Companion.rl("block/crucible_side"))
        .texture("top", Thavma.Companion.rl("block/crucible_top"))
        .texture("bottom", Thavma.Companion.rl("block/crucible_bottom"))
        .texture("side", Thavma.Companion.rl("block/crucible_side"))
        .texture("inside", Thavma.Companion.rl("block/crucible_inner"))
    ...
    this.simpleBlockWithItem(T7Blocks.INSTANCE.getESSENTIA_CONTAINER().get(), this.models().getExistingFile(Thavma.Companion.rl("essentia_container")));
    this.simpleBlockWithItem(T7Blocks.INSTANCE.getRESEARCH_TABLE().get(), this.models().getExistingFile(Thavma.Companion.rl("research_table")));

    this.itemModels().getBuilder(T7Blocks.INSTANCE.getAURA_NODE().getId().getPath()).parent(new ModelFile.UncheckedModelFile("item/generated")).renderType(RenderType.translucent().name).texture("layer0", Thavma.Companion.rl("item/aura_node"));
  }
  ...
  private void infusedBlockWithItem(InfusedBlock infusedBlock) {
    var infusedOreBlockModel = this.models().withExistingParent(this.name(infusedBlock), Thavma.Companion.rl("block/infused_stone"))
        .texture("layer0", this.blockTexture(infusedBlock.getBaseBlock()))
        .texture("layer1", Thavma.Companion.rl("block/infused_stone"));
